...
| 参数原始字段名               | UI显示名称   | word | 索引位宽     | 有符号/无符号 | UI类型   | 参数表格索引                                                                                                                  |
| ---------------------------- | ------------ | ---- | ------------ | ------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------- |
| `BIZ_TX_PARAM_COUNT_CLEAR_C` | 业务发送计数清零 | `-`  | `无 payload` | 无符号数      | 触发按钮 | [UI_BIZ_TX_CMD_GROUP_PULSE_CLEAR_C_BIZ_TX_PARAM_COUNT_CLEAR_C](#ui_biz_tx_cmd_group_pulse_clear_c_biz_tx_param_count_clear_c) |

## BIZ_TX_CMD_GROUP_PULSE_RESET_C

| 参数原始字段名         | UI显示名称   | word | 索引位宽     | 有符号/无符号 | UI类型   | 参数表格索引                                                                                                      |
| ---------------------- | ------------ | ---- | ------------ | ------------- | -------- | ----------------------------------------------------------------------------------------------------------------- |
| `BIZ_TX_PARAM_RESET_C` | 业务发送复位 | `-`  | `无 payload` | 无符号数      | 触发按钮 | [UI_BIZ_TX_CMD_GROUP_PULSE_RESET_C_BIZ_TX_PARAM_RESET_C](#ui_biz_tx_cmd_group_pulse_reset_c_biz_tx_param_reset_c) |
