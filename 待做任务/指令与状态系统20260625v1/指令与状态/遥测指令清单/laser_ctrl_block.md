...
`TM_RUNTIME_WORDS_C = 36`。前 14 个字保持原有 gdian 运行状态顺序，后续追加扫频/参数包/调制状态；原 `freq_scan_status_flags` 和 `temp_stable_status_flags` 已按 bit 拆成独立遥测 word，每个参数至少占 2 字节。
...
| 17 | `freq_param_ready` | 参数包基准已就绪，允许启动扫频/卸载 |
| 18 | `freq_baseline_ready` | 已捕获本套硬件 LO 基准码值 |
| 19 | `laser_param_recover` | 当前处于参数包恢复阶段 |
| 20 | `freq_scan_active` | 扫频/卸载正在接管 LO set |
| 21 | `freq_scan_en_status` | 遥控扫频使能有效值 |
| 22 | `freq_unload_en_status` | 遥控卸载使能有效值 |
| 23 | `yc_m195_jz_ok` | gdian 参数包校验/就绪标志 |
| 24 | `frame_lock_state` | 帧同步锁定状态 |
| 25 | `freq_scan_state` | 扫频状态机状态 |
| 26 | `last_fault_reason` | 最近一次扫频/卸载退出或禁止原因 |
| 27 | `txm1_temp_1s_stable` | TXM1 最近一次 1s 温度稳定 |
| 28 | `txm2_temp_1s_stable` | TXM2 最近一次 1s 温度稳定 |
| 29 | `lo1_temp_1s_stable` | LO1 最近一次 1s 温度稳定 |
| 30 | `lo2_temp_1s_stable` | LO2 最近一次 1s 温度稳定 |
| 31 | `txm1_temp_10s_stable` | TXM1 连续 10s 温度稳定 |
| 32 | `txm2_temp_10s_stable` | TXM2 连续 10s 温度稳定 |
| 33 | `lo1_temp_10s_stable` | LO1 连续 10s 温度稳定 |
| 34 | `lo2_temp_10s_stable` | LO2 连续 10s 温度稳定 |
| 35 | `freq_scan_offset_code` | 当前选中 LO 相对参数包基准的有符号偏移码值 |
...
`txm1_set/txm2_set/lo1_set/lo2_set`、`saomiao_on/genzong_on/flag_mod/flag_sz/dat_sz`、`doppler_on/doppler_pre` 已从配置遥测组删除；需要观察扫频接管状态时使用 runtime 组的 `freq_scan_active/freq_scan_state`。
