...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| [CXP_PARAM_RESET_C](#cxp_param_reset_c) | CXP 业务链路复位 | `8'd0` | `PARAM_KIND_PULSE_C` | `8'd0` | `0 bit` | 上电默认不触发 | `CXP_IMPL_RESET_C` | `8'd0` | `8'd1` | `32 bit` | `1 bit` | `0` |
...
| 实现层字数 | `8'd1` |
| 实现层位宽 | `32 bit` |
| 模块最终落地目标 | `reset_pulse_o`（经 `params_o.reset_pulse` 输出到模块控制链路） |
| 模块最终落地位宽 | `1 bit` |
| 模块上电默认生效值 | `0`（脉冲不触发） |
| 可选项数量 | `0` |
| 映射表 | 无 |
