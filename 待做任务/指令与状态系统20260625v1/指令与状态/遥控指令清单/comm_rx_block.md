...
| 参数名字 | 参数中文名 | 参数id | 参数类型 | 应用层字数 | 应用层位宽 | 默认值 | 实现层参数名 | 实现层参数id | 实现层字数 | 实现层位宽 | 模块最终落地位宽 | 可选项数量 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| [COMM_RX_PARAM_RANGE_RESET_C](#comm_rx_param_range_reset_c) | 测距复位触发 | `8'd10` | `PARAM_KIND_PULSE_C` | `WORD_COUNT_ZERO_C` | `0 bit` | 上电默认不触发 | `COMM_RX_IMPL_RANGE_RESET_C` | `8'd10` | `8'd1` | `32 bit` | `1 bit` | `0` |

### COMM_RX_CMD_GROUP_PULSE_COUNT_CLR_C

| 参数名字 | 参数中文名 | 参数id | 参数类型 | 应用层字数 | 应用层位宽 | 默认值 | 实现层参数名 | 实现层参数id | 实现层字数 | 实现层位宽 | 模块最终落地位宽 | 可选项数量 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| [COMM_RX_PARAM_COUNT_CLEAR_C](#comm_rx_param_count_clear_c) | 接收计数清零 | `8'd11` | `PARAM_KIND_PULSE_C` | `WORD_COUNT_ZERO_C` | `0 bit` | 上电默认不触发 | `COMM_RX_IMPL_COUNT_CLEAR_C` | `8'd11` | `8'd1` | `32 bit` | `1 bit` | `0` |

### COMM_RX_CMD_GROUP_PULSE_MANUAL_RST_C

| 参数名字 | 参数中文名 | 参数id | 参数类型 | 应用层字数 | 应用层位宽 | 默认值 | 实现层参数名 | 实现层参数id | 实现层字数 | 实现层位宽 | 模块最终落地位宽 | 可选项数量 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| [COMM_RX_PARAM_MANUAL_RESET_C](#comm_rx_param_manual_reset_c) | 手动接收复位 | `8'd12` | `PARAM_KIND_PULSE_C` | `WORD_COUNT_ZERO_C` | `0 bit` | 上电默认不触发 | `COMM_RX_IMPL_MANUAL_RESET_C` | `8'd12` | `8'd1` | `32 bit` | `1 bit` | `0` |
...
| 实现层字数 | `8'd1` |
| 实现层位宽 | `32 bit` |
| 模块最终落地目标 | `ranging_reset_pulse_o`（经 `params_o.ranging_reset_pulse` 参与 `ranging_reset_w` 生成） |
| 模块最终落地位宽 | `1 bit` |
| 模块上电默认生效值 | `0`（脉冲不触发） |
| 可选项数量 | `0` |
| 映射表 | 无 |
...
| 实现层字数 | `8'd1` |
| 实现层位宽 | `32 bit` |
| 模块最终落地目标 | `count_clear_pulse_o`（经 `params_o.count_clear_pulse` 输出到接收控制链路） |
| 模块最终落地位宽 | `1 bit` |
| 模块上电默认生效值 | `0`（脉冲不触发） |
| 可选项数量 | `0` |
| 映射表 | 无 |
...
| 实现层字数 | `8'd1` |
| 实现层位宽 | `32 bit` |
| 模块最终落地目标 | `manual_reset_pulse_o`（经 `params_o.manual_reset_pulse` 参与 `comm_rx_rst_w` 生成） |
| 模块最终落地位宽 | `1 bit` |
| 模块上电默认生效值 | `0`（脉冲不触发） |
| 可选项数量 | `0` |
| 映射表 | 无 |
