...
| 参数名字 | 参数中文名 | 参数id | 参数类型 | 应用层字数 | 应用层位宽 | 默认值 | 实现层参数名 | 实现层参数id | 实现层字数 | 实现层位宽 | 模块最终落地位宽 | 可选项数量 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| [ADC_RX_PARAM_RESET_C](#adc_rx_param_reset_c) | ADC 接收链路复位 | `8'd1` | `PARAM_KIND_PULSE_C` | `8'd0` | `0 bit` | 上电默认不触发 | `ADC_RX_IMPL_RESET_C` | `8'd1` | `8'd1` | `32 bit` | `1 bit` | `0` |
...
| 实现层字数 | `8'd1` |
| 实现层位宽 | `32 bit` |
| 模块最终落地目标 | `reset_pulse_o`（经 `params_o.reset_pulse` 送往顶层 `adc_reset_pulse_w`） |
| 模块最终落地位宽 | `1 bit` |
| 模块上电默认生效值 | `0`（脉冲不触发） |
| 可选项数量 | `0` |
| 映射表 | 无 |
