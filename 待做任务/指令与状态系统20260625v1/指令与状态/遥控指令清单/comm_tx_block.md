...
| `COMM_TX_CMD_GROUP_FAULT_C` | `8'h11` | `GROUP_FAULT_PARAM_COUNT_C = 8'd12` | `8'd3`..`8'd13`、`8'd16` | 异常注入参数配置 |
...
| 帧内容重复 | `COMM_TX_PARAM_FRAME_CONTENT_REPEAT_C` | `COMM_TX_CMD_GROUP_FAULT_C` | `8'd16` | `COMM_TX_IMPL_FRAME_CONTENT_REPEAT_C` | `8'd16` | `cfg_r.frame_content_repeat_inject` | `4 bit` | `COMM_TX_FRAME_CONTENT_REPEAT_NONE_C` |
...
| `header_error_inject` | 不注入 | `1` 帧头 `1ACFFC1D` 轻度 1bit 错误，`2` 中度 4bit 错误，`3` 反序/全反，`4` 大量错误，第 1/3 字节错误，`5` 32bit 位反序 |
...
| `data_link_break_inject` | 不注入 | `1` 星间通信数据帧时停时发/断链注入，当前由 PPS 1s 计时器控制，每 5 秒停发 1 秒，停发窗内发送 `5A5A_5A5A` |
...
| `frame_content_repeat_inject` | 不注入 | `1` 帧内容重复异常接口预留，当前仅保留遥控/遥测字段，尚未接入数据路径 |
| `endian_error_inject` | 不注入 | `1` GT 正序输出，`2` 帧头 `1ACFFC1D` 位反序，`3` 数据内容位反序，`4` 帧头与数据内容同时位反序 |
