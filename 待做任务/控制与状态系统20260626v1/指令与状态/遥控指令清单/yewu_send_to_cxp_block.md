...
| 参数名字 | 参数中文名 | 参数id | 参数类型 | 应用层字数 | 应用层位宽 | 默认值 | 实现层参数名 | 实现层参数id | 实现层字数 | 实现层位宽 | 模块最终落地位宽 | 可选项数量 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| [BIZ_TX_PARAM_COUNT_CLEAR_C](#biz_tx_param_count_clear_c) | 业务发送计数清零 | `8'd1` | `PARAM_KIND_PULSE_C` | `8'd0` | `0 bit` | 上电默认不触发 | `BIZ_TX_IMPL_COUNT_CLEAR_C` | `8'd1` | `8'd1` | `32 bit` | `1 bit` | `0` |

### BIZ_TX_CMD_GROUP_PULSE_RESET_C

| 参数名字 | 参数中文名 | 参数id | 参数类型 | 应用层字数 | 应用层位宽 | 默认值 | 实现层参数名 | 实现层参数id | 实现层字数 | 实现层位宽 | 模块最终落地位宽 | 可选项数量 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| [BIZ_TX_PARAM_RESET_C](#biz_tx_param_reset_c) | 业务发送复位 | `8'd2` | `PARAM_KIND_PULSE_C` | `8'd0` | `0 bit` | 上电默认不触发 | `BIZ_TX_IMPL_RESET_C` | `8'd2` | `8'd1` | `32 bit` | `1 bit` | `0` |
...
| 实现层字数 | `8'd1` |
| 实现层位宽 | `32 bit` |
| 模块最终落地目标 | `count_clear_pulse_o`（经 `params_o.count_clear_pulse` 输出到发送控制链路） |
| 模块最终落地位宽 | `1 bit` |
| 模块上电默认生效值 | `0`（脉冲不触发） |
| 可选项数量 | `0` |
| 映射表 | 无 |
...
| 实现层字数 | `8'd1` |
| 实现层位宽 | `32 bit` |
| 模块最终落地目标 | `reset_pulse_o`（经 `params_o.reset_pulse` 输出到发送控制链路） |
| 模块最终落地位宽 | `1 bit` |
| 模块上电默认生效值 | `0`（脉冲不触发） |
| 可选项数量 | `0` |
| 映射表 | 无 |
