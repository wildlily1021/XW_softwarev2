...
export interface ConnectionService extends ConnectionReader {
  ...
  removeConnection(connectionId: string): Promise<ConnectionOperationOutcome>;
...
export function createConnectionService(
  ...
  return {
    ...
    async removeConnection(connectionId) {
        ...
        return {
          ...
          validation: createConnectionValidationOutcome([
            createConnectionIssue(
              'connection.config.missing',
              'connectionId',
              `Connection ${connectionId} could not be found.`,
              'error',
            ),
          ]),
          snapshot: state.getSnapshot(),
      ...
      let events: readonly TransportEventSnapshot[] = [];

      if (fact.lifecycle !== 'disconnected') {
        const beforeLength = state.getSnapshot().events.length;
        ...
        if (!command.ok) {
          return {
            ...
            snapshot,
            events,
            error: adapterErrorToSnapshot(
              command.error,
              connectionId,
              lastEvent?.occurredAt ?? now(),
            ),
          };
        }
      }
      ...
      return {
        ok: true,
        validation: validValidation(),
        snapshot: state.removeConfig(connectionId),
        events,
      };
    },
