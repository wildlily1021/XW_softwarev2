...
export function createCompositeAdapter(
  ...
  return {
    ...
    async discoverResources(): Promise<readonly ConnectionResourceCandidate[]> {
      const allResources: ConnectionResourceCandidate[] = [];
      if (serialAdapter?.discoverResources) {
        allResources.push(...await serialAdapter.discoverResources());
      }
      if (networkAdapter?.discoverResources) {
        allResources.push(...await networkAdapter.discoverResources());
      }
      return allResources;
    },
  };
